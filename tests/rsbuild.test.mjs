...
test("builds and tree-shakes using rsbuild", async (t) => {
    ...
    t.test("tree shakes sync require destructuring", () => {
        assert.doesNotMatch(builtIndexCode, /SHOULD BE REMOVED FROM BUNDLE SYNC REQUIRE DESTRUCTURING/) // ❌ Throws
    })

    t.test("tree shakes sync require module", () => {
        assert.doesNotMatch(builtIndexCode, /SHOULD BE REMOVED FROM BUNDLE SYNC REQUIRE MODULE/) // ❌ Throws
    })

    t.test("tree shakes sync require chaining", () => {
        assert.doesNotMatch(builtIndexCode, /SHOULD BE REMOVED FROM BUNDLE SYNC REQUIRE CHAINING/) // ❌ Throws
    })
